...
- Vue bindings (`inertia-hotwire-native/vue`): `useBridgeComponent(name)`
  composable, with `supported` as a `Ref`.
- Svelte bindings (`inertia-hotwire-native/svelte`): `useBridgeComponent(name)`
  helper, with `supported` as a readable store.
